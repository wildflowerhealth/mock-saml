...
import { WfhEnv, WfhEnvs, getElevanceSamlConfig } from 'utils/settings';
...
export default function Elevance() {
  const router = useRouter();
  const { relayState } = router.query;

  const authUrl = '/api/saml/auth-elevance';
  ...
  const [ssoFormState, setSSOFormState] = useState<SSOFormData>({
    ...
    acsUrl: getElevanceSamlConfig(initialTargetEnv).acs,
    audience: getElevanceSamlConfig(initialTargetEnv).audience,
  });
  ...
  const [showMockEligibilityForm, setShowMockEligibilityForm] = useState(true);
  const [useReactNative, setUseReactNative] = useState(true);
  ...
  const handleChange = (e: FormEvent<HTMLInputElement | HTMLSelectElement>): void => {
    ...
    if (name === 'targetEnvironment') {
      ...
      updatedState = {
        ...
        acsUrl: getElevanceSamlConfig(targetEnv).acs,
        audience: getElevanceSamlConfig(targetEnv).audience,
      };
    } else {
      updatedState = {
        ...updatedState,
      ...
      if (typeof parsedJson === 'object' && parsedJson !== null) {
        ...
        if (
          ...
          if (!('acsUrl' in parsedJson)) {
            newState.acsUrl = getElevanceSamlConfig(targetEnv).acs;
          }
          if (!('audience' in parsedJson)) {
            newState.audience = getElevanceSamlConfig(targetEnv).audience;
          }
        }
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!useReactNative) {
      const acsCordovaWebAppDomain = getElevanceSamlConfig(
        ssoFormState.targetEnvironment
      ).acsCordovaWebAppDomain;
  ...
  return (
    <>
      <Head>
        <title>Mock Elevance Identity Provider </title>
      </Head>
        ...
        <div className='flex w-full max-w-6xl flex-col md:flex-row gap-6'>
          ...
          <div className='w-full md:w-1/2'>
            <div className='border-2 p-4 rounded-lg'>
              <h2 className='mb-5 text-center text-2xl font-bold text-gray-900'>Mock Elevance SSO</h2>
